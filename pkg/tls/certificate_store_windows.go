...
func searchCertWithStore(fac func() (certstore.Store, error), keyword string) (*tls.Certificate, error) {
	...
	log.WithoutContext().Infof("Found certificate matching keyword: [%v]", keyword)

	return &cert, nil
}

func searchCertInMachineStore(keyword string) (*tls.Certificate, error) {
	log.WithoutContext().Infof("searching for certificate containing string: [%v]", keyword)

	for _, fac := range []func() (certstore.Store, error){
		func() (certstore.Store, error) {
			log.WithoutContext().Infof("searching [%v] in keystore LocalMachine/MY", keyword)
			return certstore.OpenStoreWindows("MY", certstore.StoreLocationLocalMachine)
		},
		func() (certstore.Store, error) {
			log.WithoutContext().Infof("searching [%v] in keystore CurrentUser/MY", keyword)
